...
export default function CandidateUpload() {
  ...
  const [areaClass, setAreaClass] = useState("Rural");
  const [collegeTier, setCollegeTier] = useState("Tier 3 (State / Unranked)");
  const [firstGen, setFirstGen] = useState("Yes");
  const { processResume, isProcessing, error } = useAppContext();
  ...
  const finish = async () => {
    ...
    const success = await processResume(resumeFile, { district, areaClass, tier: collegeTier, firstGen });
    if (success) {
      navigate('/candidate')
    }
  }
          ...
          <AnimatePresence mode="wait" custom={direction}>
              ...
              >
                ...
                <div className="grid sm:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input type="text" value={district} onChange={e => setDistrict(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary" placeholder="e.g. Wayanad" />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-gray-700 mb-2">Area Classification</label>
                    <select value={areaClass} onChange={e => setAreaClass(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary appearance-none font-medium">
                      <option>Rural</option>
                  ...
                  <div>
                    ...
                    <select value={collegeTier} onChange={e => setCollegeTier(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 text-gray-900 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary appearance-none font-medium">
                  ...
                  <div>
                    ...
                    <div className="flex gap-4 mt-3">
                      <label className="flex items-center gap-2 cursor-pointer font-medium text-gray-700">
                        <input type="radio" value="Yes" checked={firstGen === 'Yes'} onChange={e => setFirstGen(e.target.value)} name="firstgen" className="w-4 h-4 text-primary focus:ring-primary" /> Yes
                      </label>
                      <label className="flex items-center gap-2 cursor-pointer font-medium text-gray-700">
                        <input type="radio" value="No" checked={firstGen === 'No'} onChange={e => setFirstGen(e.target.value)} name="firstgen" className="w-4 h-4 text-primary focus:ring-primary" /> No
                      </label>
                    </div>
                  </div>
                </div>
            ...
            {step === 3 && (
              ...
              >
                ...
                <input type="file" className="hidden" ref={resumeInputRef} onChange={(e) => setResumeFile(e.target.files[0])} accept=".pdf,.docx" />
        ...
        <div className="border-t border-gray-100 bg-gray-50/50 p-6 flex justify-between items-center px-12 z-20">
          ...
          {step < 5 ? (
            ...
            <div className="flex items-center gap-3">
              {error && <span className="text-red-500 font-bold text-sm">Error: {error}</span>}
              <Button onClick={finish} disabled={isProcessing} className="gap-2 px-8 shadow-lg shadow-primary/20">
